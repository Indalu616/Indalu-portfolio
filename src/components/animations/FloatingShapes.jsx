...
export default function FloatingShapes({ className = '' }) {
  return (
    <div className={`pointer-events-none absolute inset-0 -z-10 overflow-hidden ${className}`} aria-hidden="true">
      ...
      />
      <motion.div
        className="absolute right-[-10%] top-1/3 h-80 w-80 rounded-full bg-accent-2/20 blur-[110px]"
        animate={{ y: [0, -30, 0], x: [0, -18, 0] }}
        transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute bottom-[-10%] left-1/3 h-64 w-64 rounded-full bg-accent/10 blur-[100px]"
        animate={{ y: [0, 20, 0] }}
